...
Feature: visibility for teachers without blind marking
  ...
  Scenario: Teachers can see other grades before they are finalised when viewinitialgradeenabled is enabled
  ...
  Scenario: Teachers can not see other grades when viewinitialgradeenabled is disabled
    Given the following "activity" exists:
      | activity                | coursework |
      | course                  | C1         |
      | name                    | Coursework |
